...
- **Chat interface.** A right-panel `ItemView` rendering: streaming `text_delta` events, tool-call narration cards (`tool_call` events before and after execution), proposal cards (incoming `extension_ui_request{method:"confirm"}`), question cards (incoming `extension_ui_request{method:"select"|"input"}`), and post-hoc edit cards (incoming `extension_ui_request{method:"notify"}` with structured content).
